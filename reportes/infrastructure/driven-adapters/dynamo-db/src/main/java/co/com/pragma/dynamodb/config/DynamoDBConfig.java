...
public class DynamoDBConfig {
    ...
    @Bean
    @Profile("docker")
    public DynamoDbAsyncClient amazonDynamoDBDocker(MetricPublisher publisher,
                                                    @Value("${aws.region}") String region) {
        return DynamoDbAsyncClient.builder()
                .credentialsProvider(EnvironmentVariableCredentialsProvider.create()) // toma las de docker-compose
                .region(Region.of(region))
                .overrideConfiguration(o -> o.addMetricPublisher(publisher))
                .build();
    }
