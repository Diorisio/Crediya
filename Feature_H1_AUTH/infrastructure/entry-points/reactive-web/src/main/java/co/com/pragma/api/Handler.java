...
public class Handler {
    ...
    public Mono<ServerResponse> listenGETBuscarUsuario(ServerRequest serverRequest) {
        ...
        return useCase.existsBydocumentoIdentidad(documento)
                .flatMap(existe -> {
                    if (existe) {
                        log.info("Usuario encontrado: {}", documento);
                        return ServerResponse.ok().bodyValue("Usuario existe");
                    } else {
                        log.warn("Usuario no encontrado: {}", documento);
                        return Mono.error(new UsuarioNotFoundException(documento));
                    }
                })
